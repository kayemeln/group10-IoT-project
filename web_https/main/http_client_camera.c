...
void http_camera_post_task(void *pvParameters)
{
    ...
    esp_http_client_config_t config = {
        ...
        .timeout_ms = 15000,  // Longer timeout for TLS handshake
        .cert_pem = cert_pem_start,           // Use embedded certificate
        .cert_len = cert_pem_end - cert_pem_start,  // Certificate length
        .use_global_ca_store = false,         // Don't use global CA store
        .skip_cert_common_name_check = false, // Enable certificate name verification
    };
...
void init_http_camera_client(void)
{
    ...
    snprintf(server_url, sizeof(server_url), "https://%s%s", 
             CONFIG_HTTP_SERVER_ENDPOINT, CONFIG_HTTP_SERVER_PATH);
    ESP_LOGI(TAG, "HTTPS camera client initialized with certificate verification");
    ESP_LOGI(TAG, "Target URL: %s", server_url);
}
