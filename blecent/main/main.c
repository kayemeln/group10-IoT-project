...
/* Global variables for LED toggle functionality */
static esp_timer_handle_t led_toggle_timer = NULL;
static uint16_t connected_peer_conn_handle = BLE_HS_CONN_HANDLE_NONE;
...
#if MYNEWT_VAL(BLE_GATTC)
/**
 * Timer callback function to toggle LED every 2 seconds
 */
static void led_toggle_timer_callback(void* arg)
{
    ...
    if (connected_peer_conn_handle == BLE_HS_CONN_HANDLE_NONE) {
        ESP_LOGW(tag, "No connected peer, stopping LED toggle timer");
        if (led_toggle_timer) {
            esp_timer_stop(led_toggle_timer);
        }
        return;
    }

    peer = peer_find(connected_peer_conn_handle);
    if (peer == NULL) {
        ESP_LOGW(tag, "Peer not found, stopping LED toggle timer");
        connected_peer_conn_handle = BLE_HS_CONN_HANDLE_NONE;
        if (led_toggle_timer) {
            esp_timer_stop(led_toggle_timer);
        }
        return;
    ...
    if (chr == NULL) {
        ESP_LOGW(tag, "Custom characteristic not found");
        return;
    }

    /* Toggle LED state and send command to peripheral */
    led_state = !led_state;
    toggle_value = led_state ? 0x01 : 0x00;  /* 1 for LED ON, 0 for LED OFF */

    ESP_LOGI(tag, "Toggling LED: %s (value: 0x%02x)", 
             led_state ? "ON" : "OFF", toggle_value);

    rc = ble_gattc_write_flat(connected_peer_conn_handle, chr->chr.val_handle,
                              &toggle_value, sizeof(toggle_value), NULL, NULL);
    if (rc != 0) {
        ESP_LOGE(tag, "Failed to write LED toggle command; rc=%d", rc);
    }
}

/**
 * Start the LED toggle timer
 */
static void start_led_toggle_timer(uint16_t conn_handle)
{
    esp_err_t ret;
    
    connected_peer_conn_handle = conn_handle;
    
    if (led_toggle_timer != NULL) {
        esp_timer_stop(led_toggle_timer);
    } else {
        const esp_timer_create_args_t timer_args = {
            .callback = &led_toggle_timer_callback,
            .arg = NULL,
            .name = "led_toggle_timer"
        };
        ret = esp_timer_create(&timer_args, &led_toggle_timer);
        if (ret != ESP_OK) {
            ESP_LOGE(tag, "Failed to create LED toggle timer: %s", esp_err_to_name(ret));
            return;
        }
    }
    
    /* Start timer with 2-second interval */
    ret = esp_timer_start_periodic(led_toggle_timer, 2000000);  /* 2 seconds in microseconds */
    if (ret != ESP_OK) {
        ESP_LOGE(tag, "Failed to start LED toggle timer: %s", esp_err_to_name(ret));
    } else {
        ESP_LOGI(tag, "LED toggle timer started successfully (2-second interval)");
    }
}

/**
 * Stop the LED toggle timer (called when disconnected)
 */
static void stop_led_toggle_timer(void)
{
    if (led_toggle_timer) {
        esp_timer_stop(led_toggle_timer);
        ESP_LOGI(tag, "LED toggle timer stopped");
    }
    connected_peer_conn_handle = BLE_HS_CONN_HANDLE_NONE;
}
...
blecent_on_disc_complete(const struct peer *peer, int status, void *arg)
{
    ...
    /* Start LED toggle timer after service discovery and initial setup completes */
    start_led_toggle_timer(peer->conn_handle);
}
...
blecent_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        /* Stop LED toggle timer */
        stop_led_toggle_timer();
